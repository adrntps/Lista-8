...
alpha_val = np.linspace(0, 1, num=25)
...
for val in alpha_val:
    ...
    for k in range(10**0):
        ...
        for p in range(10):
            ...
            G_CHSH = alpha*np.kron(A0_opt, Id) + np.kron(A0_opt, B0_opt) + np.kron(A0_opt, B1_opt) + np.kron(A1_opt, B0_opt) - np.kron(A1_opt, B1_opt)
            ...
            G_CHSH_var = alpha*cp.kron(A0_opt, Id) + cp.kron(A0_var, B0_opt) + cp.kron(A0_var, B1_opt) + cp.kron(A1_var, B0_opt) - cp.kron(A1_var, B1_opt)
            ...
            G_CHSH_var_1 = alpha*cp.kron(A0_opt, Id) + cp.kron(A0_opt, B0_var) + cp.kron(A0_opt, B1_var) + cp.kron(A1_opt, B0_var) - cp.kron(A1_opt, B1_var)
